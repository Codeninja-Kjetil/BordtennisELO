...
<body>
    ...
    <p>
        <c:choose>
            <c:when test="${player.locked}">
                You are about to unlock the user "${player.username}".
                This means that the user is able to log in again.
                This action can be undone. Are you sure you want to do this?
            </c:when>
            <c:otherwise>
                You are about to lock the user "${player.username}".
    ...
    <form action="AdminLockPlayer" method="post">
        <input type="hidden" name="user" value="${player.username}" />
